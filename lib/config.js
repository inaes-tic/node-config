...
Config.prototype._schemaValidation = function() {
    ...
    if(FileSystem.existsSync(schemaValidatorFilename)) {
        try {
            schemaValidator = JSON.parse(FileSystem.readFileSync(schemaValidatorFilename, 'UTF-8'));
        }
        catch (e) {
            throw new Error('Schema Validator file ' + schemaValidatorFilename + ' cannot be read');
        }
        if(schemaValidator) {
            jsonSchemaValidator.validate(t , schemaValidator, function(errs) {
